...
#** 2. Tune growth rate ============================================================
# We start by increasing the constant in the maximum consumption rate by a factor of 1.7 (1.5 with erepro = 0.1)
# If growth is more reasonable, tune down kappa (higher consumption allows for lower kappa to get coexistence). I tuned it to 6 (2 with erepro = 0.1 across), from 8 with erepro = 0.1, and from 5 with erepro = allometric in step 1, which gives reasonable SSB given the new consumption rates
params2 <- params
...
params2@species_params$h <- params@species_params$h * 1.7
...
# After improving growth, reduce kappa from step one sequntially until coexistence is not possible and increase slightly. I landed on kappa = 2. The SSB's will be optimized in the next step.
...
# [1]  0.006831648 91.596277228  6.789587547
# with allometric erepro (and h-scaling factor = 1.7):
# > exp(optim_resultNC$`par`)
# [1] 0.00437186 3.86949769 0.41864194
...
plotGrowthCurves(m3) + 
  geom_hline(data = empiri, aes(yintercept = w_mat), 
             color = "gray30", size = 1, linetype = 2) +
  geom_line(data = empiri, aes(age, weight), 
            color = col[3], size = 1.8, linetype = "twodash") +
  geom_line(aes(x = Age, y = value), 
...
m4@params@linecolour[1:3] <- col[c(1,5,3)]
...
# growth rates became OK after increasing Cmax by 70%
...
# Yet, the predicted dynamics are changing less compared to observed, and in general it doesn't capture regime shifts (amplitude is off). Is this because we don't have as strong interactions as in the system(lots off background feeding and response to F mainly)? What can we do about that? Those things should be addressed perhaps already in the diet stage. How about reducing the cutoff size of background?

# *** Update this based on to do list
