...
all_effort_sum %>% 
  mutate(mean = mean(sum_NomActive),
         sd = sd(sum_NomActive), .by = "sp") %>% 
  mutate(extreme = ifelse(sum_NomActive > mean + 5*sd, "extreme", "not extreme")) %>% 
  ggplot(aes(Year, sum_NomActive, color = extreme)) + 
  #geom_bar(stat = "identity") +
  #geom_line() +
  geom_point() +
...
sums <- d %>% 
  summarise(mean = mean(assessment - fish_mip_assessment_f_max),
            median = median(assessment - fish_mip_assessment_f_max), .by = "sp")
...
projectEffort <- projectEffort %>% 
  ...
  mutate(effort = ifelse(effort < 0, 0.0001, effort))
...
# Check average deviation from assessment:
projectEffort %>% 
  filter(year >= 1992 & year <= 2004) %>% 
  dplyr::select(-source2) %>% 
  pivot_wider(names_from = source, values_from = effort) %>% 
  group_by(sp) %>% 
  summarise(mean_assess = mean(`FishMIP_assessment F max` - assessment))
  # summarise(mean_assess = mean(assessment), 
  #           mean_fmip = mean(`FishMIP_assessment F max`),
  #           .by = "sp") %>% 
  # ungroup() %>% 
  # summarise(mean = mean_assess / mean_fmip)
