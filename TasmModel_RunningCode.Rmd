...
Centrstephanus - Wmax = 700g, Wmat = 65g with spines. For the model purpose therefore Wmax = 500g, Wmat = 45g
...
mariaParams <- read.csv(file = "Tasm_Parameters.csv") #16 species
...
## resource params
kappa = 1 # 0.02 # 1.5 #1.5 ## 0.017g/m3 
lambda = 2.0 #1.9 #2.1 
w_pp_cutoff = 1 #5
r_pp = 2 
min_w_pp = 1e-10

kappa_ben = 14 #0.035 # 0.025 and 0.017 worked, but not enough benthos in diets # 1.3 #2, most parameterisation at 1, but try higher
lambda_ben = 1.6 #0.6 #1.6 #1.8 used
...
kappa_alg = 20 # 1.5 #1 # 0.02 # 1 #2
lambda_alg = 1.5 #0.5 # 1.7
w_aa_cutoff = 100 
r_aa = 1 #0.2
min_w_aa = 0.01
...
obsab <- 1.1 - 0.62*log10(params@w_full)

# the model abundance is per m3, but observations are per m2. I assume that depth is ca 10m, so the model abundance should be multipled by 10
#length(params@dw_full[c(min(which(params@w_full > min_w_bb)):max(which(params@w_full < w_bb_cutoff)))])

modab <- log10(params@initial_n_bb * params@dw_full * 10) 
modpl <- log10(params@initial_n_pp * params@dw_full * 10)
modaa <- log10(params@initial_n_aa * params@dw_full * 10)

plot(log10(params@w_full), modab, type = 'l', xlim = c(-6,2), ylim = c(-3, 5), main = "Slopes of initial benthic abundance versus observed abundance", xlab = "Log10, w, g", ylab = "Log10(Abundance per m2")
points(log10(params@w_full), obsab, type = 'l', col = 'red')
points(log10(params@w_full),modpl, type = 'l', col = 'green')
points(log10(params@w_full),modaa, type = 'l', col = 'brown')
#abline(v = log10(0.05), lty = 2)
abline(v = log10(min_w_bb), lty =2)

#Note, the slope of the model abundance is not as steep, because we actually get a lot of depletion at larger sizes and the realised abundance at larger sizes is much lower. The observed abundances should more closely match the realised abundances  
...
# then for herbivores, who have low assimilation efficiency, we increase the maximum intake rate 3 times (based on Graham Edgar daily intake calculations, ref in Soler et al. 2018)
...
# for predators gamma values should be much higher or else they don't get enough food 
gammaDef[which(mariaParams$funcgr == "predat")] <- gammaDef[which(mariaParams$funcgr == "predat")] *4
mariaParams$gamma <- gammaDef
...
# multiply initial abundances by the relative abundance of this species in the observations, which is the abund_scalar calculated above
...
#now run steady function again (above)
...
mariaParams$erepro_2 <- stedTas@species_params$erepro
...
Freddie <- read.csv(file = "C:/Users/astaa/Dropbox/asta/TasmanianModel/Parameters/BenthicInvertData/Freddie.csv")

Kate <- read.csv(file = "C:/Users/astaa/Dropbox/asta/TasmanianModel/Parameters/BenthicInvertData/all_bysample_size.csv")
KateCodes <- read.csv(file = "C:/Users/astaa/Dropbox/asta/TasmanianModel/Parameters/BenthicInvertData/sample_data_env_vectors.csv")

# coordinates for eastern Tasmanian coast
#TasmCoorLon <- c(146.5, 148.5) 
#TasmCoorLat <- c(-41.00, -43.60)
KateWgtGroups <- c(1.8E-06,4.8E-06, 1.1E-05, 2.9E-05, 7.1E-05, 1.8E-04, 4.5E-04, 1.1E-03, 2.8E-03, 6.8E-03, 1.7E-02, 4.2E-02, 1.1E-01, 2.5E-01, 6.8E-01, 1.6E+00)
...
#save(InvData, file = "BenticInvAbund_Tasm.RData")
...
breaks <- seq(from = -6, to = 4) #make a vector of breaks 
...
plot(log10(InvData$wgt), log10(InvData$meanAb), type = 'l', ylim = c(-4, 6), main = "Benthos slopes in Tasmania: log(Abund) = 0.35 - 0.71*log(wgt)", xlab = "Log10 (w,g)", ylab = "Log10 (Abundance: min, mean, max)")
...
bs <- lm(log10(InvData$meanAb) ~ log10(InvData$wgt))
